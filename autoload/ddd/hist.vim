...
function ddd#hist#expand(expand_wild) abort
  ...
  call ddd#log#debug(printf('parse cmdline into : %s', string(cs)))
...
function s:set_last_string_search(s, cmdtype) abort
  ...
  let s:last_search[a:cmdtype] = a:s
endfunction
...
function s:use_vim_regex_search() abort
  return get(g:, 'ddd_hist_use_vim_regex_search', 0)
endfunction
...
function s:expand_exclamation(cs) abort
  ...
  call ddd#log#debug(printf('expand event to %s', s))

  if !empty(s) && !empty(a:cs.word)
    let s = s:expand_word(a:cs, s)
    call ddd#log#debug(printf('expand word to %s', s))
  endif

  if !empty(s) && !empty(a:cs.modifier)
    let s = s:expand_modifier(a:cs, s)
    call ddd#log#debug(printf('expand modifier to %s', s))
  endif
