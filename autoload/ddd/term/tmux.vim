let s:term = deepcopy(ddd#term#new())

function ddd#term#tmux#new() abort
...
function ddd#term#tmux#spawn(opts) abort
  let term = ddd#term#tmux#new()
...
function s:term.split() abort
  ...
  if has_key(self, 'pane')
    ...
    let self.vpane = ddd#tmux#pane()
    let self.pane = ddd#tmux#split(cmd)
  endif
