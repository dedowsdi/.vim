...
function ddd#to#sel(pat, ai) abort
  ...
  try
    let spat = @/
    let @/ = a:pat
    call ddd#log#debug( printf( 'sel : %s', @/ ) )
    norm! gn
    if mode() !~# "[vV\<c-v>]"
      return 0
    endif
...
function ddd#to#sel_letter(ai) abort
  return ddd#to#sel( '\v[a-zA-Z]*%#[a-zA-Z]+', a:ai )
endfunction

function ddd#to#sel_number(ai) abort
  return ddd#to#sel( '\v[0-9.\-]*%#[0-9.\-]+', a:ai )
endfunction

" TODO implement a?
function ddd#to#column(ai) abort
  call s:assert_start_state()
  exec "norm! \<c-v>"
  if dddu#get_cc() !~# '\s'
    call ddd#mo#vertical_motion('E')
  else
    " doesn't exist or is blank
    call ddd#mo#vertical_motion('S')
    if dddu#get_cc() !~# '\s'
      norm! k
    endif
  endif
...
function ddd#to#sel_lines(pattern0, pattern1, ai, style)
  call s:assert_start_state()
