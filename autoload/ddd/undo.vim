...
" [, seq]
function ddd#undo#tag(force, name, ...) abort
...
function ddd#undo#tag_start() abort
  call extend( s:get_buf_tags(), { 'start_tag' : changenr() }, 'keep' )
endfunction

function ddd#undo#undostart() abort
  call ddd#undo#checkout('start_tag')
endfunction

" warning when you try to undo into older sessions
function ddd#undo#safeundo() abort
  ...
  if changenr() == ddd#undo#get_tag('start_tag')
    echohl ErrorMsg
...
function ddd#undo#list_tags() abort
  let buf_tags = s:get_buf_tags()
...
function ddd#undo#checkout(name) abort
  let buf_tags = s:get_buf_tags()
...
function ddd#undo#complete_tag(arglead, cmdline, pos) abort
  let buf_tags = s:get_buf_tags()
  return filter(keys(buf_tags), {i,v -> v =~# '\V\^' . a:arglead })
endfunction

function ddd#undo#get_tag(name, ...)
...
"
function ddd#undo#undo_branch_point() abort
