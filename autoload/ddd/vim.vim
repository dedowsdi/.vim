...
"
" [lnum]
function ddd#vim#goto_function(...) abort
  let lnum = get(a:000, 0, 0)
  let range = ddd#vim#get_function_range()
...
function ddd#vim#get_function_range() abort
  try
    let cview = winsaveview()
    call ddd#vim#sel_func('i')
    exec "norm! \<esc>"
  finally
...
function ddd#vim#sel_func(ai)
  call ddd#to#sel_lines('\v^\s*fu%[nction]?\!?\s+\S+\(',
...
function ddd#vim#break_here() abort
  try
    let cpos = getcurpos() 
    let cview = winsaveview()
    if ddd#vim#goto_function()

      let [scope, func_name] = matchlist(getline('.'), '\v(\w:)?(\k+)\ze\s*\(.*\)')[1:2]
      let break_line = cpos[1] - line('.')
      if scope ==# 's:'
        "add <SNR>SID_ prefix
        let func_name = '<SNR>'.ddd#vim#get_sid(@%).'_'.func_name
      endif
    else
...
function ddd#vim#break_numbered_function() abort
  let range = ddd#vim#get_function_range()
  if range != []
    let candidates = ddd#vim#search_numbered_functions(
...
" return {fnum : def}
function ddd#vim#search_numbered_functions(func_def, max_number)
...
function ddd#vim#is_comment(line_string) abort
  return matchstr(a:line_string, '\v^\s*\zs\S') ==# '"'
endfunction

function ddd#vim#is_scope_script() abort
  return stridx(getline('.'), 's:') >= 0
endfunction

" [+-][size]
function ddd#vim#list(sfile, slnum, ...) abort
